...
func createGroupRecord(ctx context.Context, client *dbent.Client, groupIn *service.Group) error {
	...
	videoModelPrices := videoModelPricesForPersistence(groupIn)
	builder := client.Group.Create().
		SetName(groupIn.Name).
		SetDescription(groupIn.Description).
		SetPlatform(groupIn.Platform).
		SetRateMultiplier(groupIn.RateMultiplier).
		SetSafeRateMultiplier(groupIn.SafeRateMultiplier).
		...
		SetVideoModelPrices(videoModelPrices).
		SetVideoBillingUnit(groupIn.EffectiveVideoBillingUnit()).
