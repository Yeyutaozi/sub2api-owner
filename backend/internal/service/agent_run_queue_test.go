...
func TestAgentRunServiceDispatchLeasePreventsConcurrentOwners(t *testing.T) {
	rdb, cleanup := newAgentRunQueueTestRedis(t)
	defer cleanup()

	svc := NewAgentRunService(nil, nil, nil, nil, nil, disabledAgentArtifactStore{}, nil)
	svc.redisClient = rdb
	ttl := agentRunDispatchLeaseTTL(agentRunQueueClaimIdle + time.Hour)

	token, acquired, err := svc.acquireAgentRunDispatchLease(context.Background(), 88, ttl)
	require.NoError(t, err)
	require.True(t, acquired)
	require.NotEmpty(t, token)
	require.Greater(t, rdb.TTL(context.Background(), agentRunDispatchLeaseKey(88)).Val(), agentRunQueueClaimIdle)

	_, acquired, err = svc.acquireAgentRunDispatchLease(context.Background(), 88, ttl)
	require.NoError(t, err)
	require.False(t, acquired)

	svc.releaseAgentRunDispatchLease(88, "not-the-owner")
	require.Equal(t, token, rdb.Get(context.Background(), agentRunDispatchLeaseKey(88)).Val())

	svc.releaseAgentRunDispatchLease(88, token)
	replacementToken, acquired, err := svc.acquireAgentRunDispatchLease(context.Background(), 88, ttl)
	require.NoError(t, err)
	require.True(t, acquired)
	require.NotEqual(t, token, replacementToken)
	svc.releaseAgentRunDispatchLease(88, replacementToken)
}

func TestAgentRunServiceHandleQueueMessageRetriesAfterDispatchLeaseRelease(t *testing.T) {
	...
	hostID := int64(89)
	run := newAgentRunQueueDispatchTestRun(188, hostID)
	repo := &testAgentRunRepo{run: run}
	svc := newAgentRunQueueDispatchTestService(rdb, repo, run, workerServer.URL)
	message := enqueuePendingAgentRunQueueMessage(t, svc, rdb, run.ID, "ar_lease_retry", workerServer.URL)

	leaseToken, acquired, err := svc.acquireAgentRunDispatchLease(context.Background(), run.ID, agentRunDispatchLeaseTTL(time.Hour))
	require.NoError(t, err)
	require.True(t, acquired)

	svc.handleAgentRunQueueMessage(context.Background(), "blocked-consumer", message)
	requireAgentRunQueueMessagePending(t, rdb, message.ID, true)
	select {
	case <-workerCalled:
		t.Fatal("worker was called while another runner owned the dispatch lease")
	default:
	}

	svc.releaseAgentRunDispatchLease(run.ID, leaseToken)
	svc.handleAgentRunQueueMessage(context.Background(), "retry-consumer", message)

	select {
	case <-workerCalled:
	default:
		t.Fatal("worker was not called after the dispatch lease was released")
	}
	requireAgentRunQueueMessagePending(t, rdb, message.ID, false)
}

func TestAgentRunServiceHandleQueueMessageRejectsUnsupportedWorkerAuthType(t *testing.T) {
	workerCalled := make(chan struct{}, 1)
	workerServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		workerCalled <- struct{}{}
		w.Header().Set("Content-Type", "application/json")
	...
	hostID := int64(90)
	run := newAgentRunQueueDispatchTestRun(189, hostID)
	repo := &agentRunQueueResponseRepo{testAgentRunRepo: &testAgentRunRepo{run: run}}
	svc := newAgentRunQueueDispatchTestService(rdb, repo, run, workerServer.URL)
	svc.workerHostRepo.(*testCancelWorkerHostRepo).host.AuthType = AgentWorkerAuthBearer
	message := enqueuePendingAgentRunQueueMessage(t, svc, rdb, run.ID, "ar_auth_rejected", workerServer.URL)

	svc.handleAgentRunQueueMessage(context.Background(), "auth-consumer", message)

	select {
	case <-workerCalled:
		t.Fatal("worker was called with an unsupported auth type")
	default:
	}
	require.Equal(t, "WORKER_AUTH_TYPE_UNSUPPORTED", repo.failedCode)
	requireAgentRunQueueMessagePending(t, rdb, message.ID, false)
}
