...
func (s *AgentRunService) CreateRun(ctx context.Context, input CreateAgentRunInput) (*AgentRun, error) {
	...
	if err := s.enqueueRun(ctx, run.ID, runToken, callbackBaseURL); err != nil {
		...
		go s.dispatchRun(run.ID, runToken, callbackBaseURL)
	}
...
func (s *AgentRunService) dispatchRun(runID int64, runToken, callbackBaseURL string) (acknowledge bool) {
	ctx := context.Background()
	...
	timeout := time.Duration(host.TimeoutSeconds)*time.Second + defaultAgentWorkerHTTPGrace
