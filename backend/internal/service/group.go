...
// IsGroupBindableInSimpleMode is the shared policy for simple-mode group binding.
...
func (g *Group) EffectiveVideoBillingUnit() string {
	if g == nil {
...
func (g *Group) EffectiveVideoBillingUnitForModel(model string) string {
	...
	if IsVideoModelPricingPlatform(g.Platform) && len(g.VideoModelPrices) > 0 {
...
func (g *Group) HasExplicitVideoModelPrice(model string) bool {
	if g == nil || !IsVideoModelPricingPlatform(g.Platform) || len(g.VideoModelPrices) == 0 {
		return false
	}
