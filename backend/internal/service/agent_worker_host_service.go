...
func buildWorkerHostFromInput(input CreateAgentWorkerHostInput, current *AgentWorkerHost) (*AgentWorkerHost, error) {
	...
	authType, err := supportedAgentWorkerAuthType(input.AuthType)
	if err != nil {
		return nil, err
	}
...
func supportedAgentWorkerAuthType(value string) (string, error) {
	authType := strings.TrimSpace(value)
	if authType == "" {
		authType = AgentWorkerAuthHMACRunToken
	}
	if authType != AgentWorkerAuthHMACRunToken {
		return "", infraerrors.BadRequest("AGENT_WORKER_AUTH_TYPE_UNSUPPORTED", "当前仅支持 HMAC + Run Token 鉴权")
	}
	return authType, nil
}
