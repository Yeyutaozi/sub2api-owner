...
const apiKeyAuthSnapshotVersion = 21 // v21: force refresh snapshots after model-specific video pricing fix
...
func isSupportedAPIKeyAuthSnapshot(snapshot *APIKeyAuthSnapshot) bool {
	return snapshot != nil && snapshot.Version == apiKeyAuthSnapshotVersion
}
...
func (s *APIKeyService) snapshotFromAPIKey(ctx context.Context, apiKey *APIKey) *APIKeyAuthSnapshot {
	...
	if apiKey.Group != nil {
		...
		if IsVideoModelPricingPlatform(apiKey.Group.Platform) {
...
func (s *APIKeyService) snapshotToAPIKey(key string, snapshot *APIKeyAuthSnapshot) *APIKey {
	...
	if snapshot.Group != nil {
		videoModelPrices := VideoModelPrices{}
		if IsVideoModelPricingPlatform(snapshot.Group.Platform) {
		...
		apiKey.Group = &Group{
			...
			VideoModelPrices:                videoModelPrices,
			VideoBillingUnit:                snapshot.Group.VideoBillingUnit,
