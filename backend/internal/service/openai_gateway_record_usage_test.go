...
func TestOpenAIGatewayServiceRecordSeedanceUsage_UsesStableTaskID(t *testing.T) {
	...
	require.NoError(t, err)
	require.NotNil(t, billingRepo.lastCmd)
	require.Equal(t, "seedance:vidjob_123", billingRepo.lastCmd.RequestID)
	require.NotNil(t, usageRepo.lastLog)
	require.Equal(t, "seedance:vidjob_123", usageRepo.lastLog.RequestID)
}
	...
	err := svc.RecordUsage(context.Background(), &OpenAIRecordUsageInput{
		...
		APIKey: &APIKey{
			...
			Group: &Group{
				...
				VideoModelPrices: VideoModelPrices{
					VideoPriceFamilyGrokImagineVideo15: {Price720P: &modelVideoPrice720P},
				},
			},
		},
		User:    &User{ID: 20129},
