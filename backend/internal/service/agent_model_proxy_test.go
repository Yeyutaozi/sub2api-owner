...
func TestAgentModelProxyGatewayCallerPropagatesAgentUsageHeaders(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		require.True(t, VerifyAgentModelProxyInternalRequest(r))
		var upstreamRequest map[string]any
		require.NoError(t, json.NewDecoder(r.Body).Decode(&upstreamRequest))
...
func TestAgentModelProxyInternalAuthRejectsHeaderTampering(t *testing.T) {
	req := httptest.NewRequest(http.MethodPost, "/v1/chat/completions", nil)
	req.Header.Set("User-Agent", "Sub2API-Agent-ModelProxy/1.0")
	req.Header.Set("X-Sub2API-Agent-App-ID", "11")
	req.Header.Set("X-Sub2API-Agent-Run-ID", "33")
	SignAgentModelProxyInternalRequest(req)
	require.True(t, VerifyAgentModelProxyInternalRequest(req))

	req.Header.Set("X-Sub2API-Agent-Run-ID", "34")
	require.False(t, VerifyAgentModelProxyInternalRequest(req))
}
