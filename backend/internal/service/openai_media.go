	...
	if mediaType == "multipart/form-data" {
		...
		if parsed.IsVideoCreate() {
			applyOpenAIVideoBillingMetadata(
				parsed,
				firstNonEmptyString(gjson.GetBytes(body, "size").String(), gjson.GetBytes(body, "resolution").String()),
				firstNonEmptyString(gjson.GetBytes(body, "seconds").String(), gjson.GetBytes(body, "duration").String()),
			)
		}
	}
	if err != nil {
...
func parseOpenAIMediaMultipartModel(body []byte, contentType string) (string, error) {
	fields, err := parseOpenAIMediaMultipartFields(body, contentType, "model")
	return fields["model"], err
}

func parseOpenAIMediaMultipartFields(body []byte, contentType string, fieldNames ...string) (map[string]string, error) {
	...
	for {
		...
		if _, ok := wanted[name]; !ok || part.FileName() != "" {
		...
		if _, exists := fields[name]; exists {
			return nil, fmt.Errorf("multipart field %s must be provided once", name)
		}
...
func applyOpenAIVideoBillingMetadata(parsed *OpenAIMediaRequest, rawResolution string, rawDuration string) {
	if parsed == nil || !parsed.IsVideoCreate() {
		return
	}
	resolution := strings.ToLower(strings.TrimSpace(rawResolution))
	switch {
	case strings.Contains(resolution, "1080"), strings.Contains(resolution, "1024"), strings.Contains(resolution, "1792"):
		parsed.VideoResolution = VideoBillingResolution1080P
	case strings.Contains(resolution, "720"), strings.Contains(resolution, "1280"):
		parsed.VideoResolution = VideoBillingResolution720P
	case strings.Contains(resolution, "480"):
		parsed.VideoResolution = VideoBillingResolution480P
	case resolution != "":
		parsed.VideoResolution = NormalizeVideoBillingResolutionOrDefault(resolution)
	}
	if duration, err := strconv.Atoi(strings.TrimSpace(rawDuration)); err == nil {
		parsed.VideoDurationSecs = NormalizeVideoBillingDurationSecondsOrDefault(duration)
	}
}
...
func (s *OpenAIGatewayService) ForwardMedia(
	...
	SetActualOpenAIUpstreamEndpoint(c, parsed.Endpoint)
	...
	if parsed.IsVideoCreate() {
		result.VideoCount = 1
		result.VideoResolution = parsed.VideoResolution
		result.VideoDurationSeconds = parsed.VideoDurationSecs
	}
	contentType := strings.TrimSpace(resp.Header.Get("Content-Type"))
