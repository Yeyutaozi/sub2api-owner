...
func TestCalculateOpenAIRecordUsageCostWebSearchPerCall(t *testing.T) {
	...
	cost, err := svc.calculateOpenAIRecordUsageCost(context.Background(), result, apiKey, result.Model, []string{"gpt-5.6-sol"}, 3.0, 1.0, 1.0, 2.0, UsageTokens{}, "", false)
	...
	cost, err = svc.calculateOpenAIRecordUsageCost(context.Background(), result, apiKey, result.Model, []string{"gpt-5.6-sol"}, 1.0, 1.0, 1.0, 1.0, UsageTokens{}, "", false)
	require.NoError(t, err)
	...
	_, err = svc.calculateOpenAIRecordUsageCost(context.Background(), result, apiKey, result.Model, []string{"gpt-5.6-sol"}, 1.0, 1.0, 1.0, 1.0, UsageTokens{InputTokens: 10}, "", false)
	require.Error(t, err)
}
