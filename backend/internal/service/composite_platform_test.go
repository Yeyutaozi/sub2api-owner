...
func TestDetectModelPlatform(t *testing.T) {
	tests := []struct {
		...
		{name: "glm", model: "glm-5", platform: PlatformGLM, ok: true},
		{name: "zhipu prefix", model: "zhipuai/glm-4.5", platform: PlatformGLM, ok: true},
...
func TestCompositeGroupSchedulerHasAllCanonicalPlatformBuckets(t *testing.T) {
	...
	require.ElementsMatch(t,
		[]string{PlatformAnthropic, PlatformGemini, PlatformOpenAI, PlatformAntigravity, PlatformGrok, PlatformGLM, PlatformSeedance, PlatformLTX, PlatformHappyHorse, PlatformMiniMax, PlatformGrokImagine},
		platforms,
	)
}
