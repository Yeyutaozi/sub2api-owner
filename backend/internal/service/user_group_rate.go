...
type GroupVideoModelPricesInput struct {
	UserID           int64            `json:"user_id"`
	VideoModelPrices VideoModelPrices `json:"video_model_prices"`
}
...
type UserGroupRateRepository interface {
	...
	SyncGroupVideoModelPrices(ctx context.Context, groupID int64, entries []GroupVideoModelPricesInput) error

	// ClearGroupRPMOverrides 清空指定分组的所有 rpm_override（整组 rpm 部分归 NULL）
	ClearGroupRPMOverrides(ctx context.Context, groupID int64) error

	ClearGroupVideoModelPrices(ctx context.Context, groupID int64) error
