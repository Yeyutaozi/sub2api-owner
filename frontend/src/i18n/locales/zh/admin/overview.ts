export default {
    ...
    groups: {
      ...
      form: {
        ...
        rateMultiplierLabel: '售卖倍率',
        rateMultiplierHint: '面向用户的售卖倍率（1.0 = 标准费率，0.5 = 半价，2.0 = 双倍）。与安全倍率独立。',
        safeRateMultiplierLabel: '安全倍率',
        safeRateMultiplierHint: '该分组独立的上游成本上限。上游成本高于此值的账号会在本分组被切断调度。不与售卖倍率、用户专属售卖倍率绑定。',
        rpmLimit: '每分钟请求数 (RPM)',
        rpmLimitPlaceholder: '0 表示不限制',
      ...
      userPricingTitle: '用户专属计费管理',
      userPricingSaved: '用户专属计费已保存',
      addUserRate: '添加用户专属倍率',
      videoPriceOverrides: {
        addUser: '添加用户专属计费',
        optionalRate: '倍率（高级可选）',
        optionalRatePlaceholder: '不填则不覆盖分组倍率',
        addAndConfigure: '添加并配置单价',
        addUserHint: '请先从下拉列表点选用户（不要只粘贴邮箱）。添加后会自动展开「模型/清晰度单价」表格。',
        workflowHint: '正确用法：搜索并点选用户 → 点「添加并配置单价」→ 在展开表格里填写模型/清晰度绝对单价 → 保存。倍率只是可选高级项，不是主要能力。',
        groupDefaultsTitle: '当前分组默认模型单价',
        groupDefaultsEmpty: '该分组尚未配置任何模型默认单价；仍可为用户填写绝对单价，或先到分组编辑页配置默认价。',
        configuredUsers: '已设置专属计费的用户',
        empty: '暂无用户设置专属模型单价（或倍率）。添加用户后请配置模型/清晰度单价。',
        column: '模型单价',
        configure: '配置模型单价',
        collapse: '收起单价表',
        notSetYet: '尚未设置',
        ...
        inheritPrice: '继承 ${price}/{unit}',
        notConfigured: '分组未配置',
        invalidPrice: '视频单价必须是大于或等于 0 的数字',
        clearUser: '清空该用户视频单价',
      },
      rpmOverrides: '专属 RPM',
      ...
      videoPricing: {
        ...
        priceUnitPerRequest: '$/次',
        pricePeriodPerSecond: '秒',
        pricePeriodPerRequest: '次',
        ...
        notConfigured: '未配置',
        validation: {
          modelRequired: '请填写第 {row} 行的模型名称。',
          duplicateModel: '模型“{model}”配置了多次。',
          invalidPrice: '模型“{model}”包含无效价格，价格必须大于或等于 0。',
          priceRequired: '请为模型“{model}”至少配置一个分辨率价格，或删除该行。'
        }
      },
      webSearchPricing: {
        title: 'Codex 网页搜索计费',
