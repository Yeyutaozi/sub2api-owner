      ...
      <div v-if="form.type === 'apikey' && form.platform !== 'antigravity'" class="space-y-4">
        <div v-if="form.platform === 'seedance' || form.platform === 'minimax'">
          ...
          >
            ...
            <option v-if="form.platform === 'seedance'" value="openvideo">{{ t('admin.accounts.videoProvider.openvideo') }}</option>
            <option v-if="form.platform === 'seedance'" value="zhi168">{{ t('admin.accounts.videoProvider.zhi168') }}</option>
            <option v-if="form.platform === 'seedance'" value="tianyue">{{ t('admin.accounts.videoProvider.tianyue') }}</option>
          </select>
        ...
        >
          <label class="flex items-start gap-2">
            ...
            <span><span class="input-label !mb-0">{{ t('admin.accounts.videoProvider.lingdongMappingEnabled') }}</span></span>
          </label>
          <template v-if="lingdongMappingEnabled">
            <input v-model="lingdongApiKey" type="password" class="input" autocomplete="new-password" data-testid="create-lingdong-api-key" :placeholder="t('admin.accounts.videoProvider.lingdongApiKeyPlaceholder')" />
            <input v-model="lingdongBaseUrl" type="text" class="input" data-testid="create-lingdong-base-url" :placeholder="t('admin.accounts.videoProvider.lingdongBaseUrlPlaceholder')" />
            <input v-model="lingdongUpstreamModel" type="text" class="input" data-testid="create-lingdong-upstream-model" :placeholder="t('admin.accounts.videoProvider.lingdongUpstreamModelPlaceholder')" />
            <div class="flex justify-end">
              <button type="button" class="text-xs font-medium text-primary-600" @click="addLingdongModelMappingRow">{{ t('admin.accounts.videoProvider.lingdongModelMappingAdd') }}</button>
            </div>
...
const isVideoAccountPlatform = computed(() =>
  ['seedance', 'ltx', 'happyhorse', 'minimax', 'grokimagine'].includes(form.platform)
)
