              ...
              <div v-if="runUsageLogs.length" class="mt-4 rounded-lg border border-gray-200 p-4 dark:border-dark-700">
                ...
                <div class="mt-3 space-y-2">
                  <div v-for="log in runUsageLogs" :key="log.id" class="flex items-center justify-between gap-3 text-xs text-gray-600 dark:text-gray-300">
                    ...
                    <span class="flex-shrink-0">{{ usageLogTokens(log) }} Token · ${{ Number(log.actual_cost || 0).toFixed(6) }}</span>
                  </div>
                </div>
              </div>
...
async function loadRunUsage(runId: number, options: { silent?: boolean } = {}) {
  try {
    const data = await usageAPI.listByAgentRun(runId)
    runUsageLogs.value = data.items
    if (data.items.length === 0 && isTerminalRunStatus(selectedRun.value?.status) && runUsageRetryCount < 3) {
      runUsageRetryCount++
...
function handleInputFilesSelected(field: InputFieldItem, event: Event) {
  ...
  if (field.kind === 'image' && selected.some(file => !file.type.startsWith('image/'))) {
    appStore.showError('参考图片只允许选择图片文件')
    target.value = ''
    return
  }
  if (field.kind === 'audio' && selected.some(file => !file.type.startsWith('audio/'))) {
    appStore.showError('该输入项只允许选择音频文件')
    target.value = ''
    return
  }
  if (field.kind === 'video' && selected.some(file => !file.type.startsWith('video/'))) {
...
function normalizeInputField(name: string, value: unknown, required: boolean): InputFieldItem | null {
  ...
  const kind = normalizeInputKind(stringValue(record['x-input-kind']), stringValue(record.type), Array.isArray(record.enum))
  if (!kind) return null
  return {
    name,
...
function normalizeInputKind(inputKind: string, schemaType: string, hasEnum = false): InputFieldKind | null {
  if (['textarea', 'text', 'number', 'image', 'file', 'audio', 'video', 'select', 'boolean', 'date'].includes(inputKind)) {
    return inputKind as InputFieldKind
  }
  if (schemaType === 'boolean') return 'boolean'
  if (schemaType === 'number' || schemaType === 'integer') return 'number'
  if (schemaType === 'array') return 'file'
  if (schemaType === 'string' && hasEnum) return 'select'
